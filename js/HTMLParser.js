...
class TNHTMLScraper {
    ...
    */
	...
	this._request.onload = function() { //now 'this' refers to the XMLHttpRequest
	    ...
	    _this.bookAbbreviations = getItemsBehindLink(this.response); /* save the abbreviations to the HTMLScraper object so we can
									    retrieve them later
									 */
	    if (setterFunction) {
		setterFunction(_this.bookAbbreviations); //this is supposed to call a callback that will assign the return value
	    }
	...
	this._request.onerror = errorFunction; //set the error function

	this._request.open('get', BASE_LINK + TN_LINK, true);
	this._request.send(); //send the request and call 'onload' when finished
